...
service.exchangeCoupon = function(coupon, shop, openId) {
    ...
    return Promise.all([getShop, getCoupon]).then(function(promises) {
        ...
        return data("exchange").count({
            shopCode: shop,
            couponCode: {$regex: "a.*", $options: '-i'}
        }).then(function(count) {
            if (count < targetShop.initNumber || targetCoupon.type !== 'bag') {
                ...
                var queue = [addExchange, expireCoupon];
                (targetCoupon.type === 'bag') && (queue.push(data("shop").findOneAndUpdate({code: shop}, {$inc: {remainingNumber: -1}})));
